...
#read the file
library(readr)
df <- read_csv("Peak.csv")
df <- data.frame(df[-1,])
row.names(df) <- df[,1]
df <- df[,-1]

#select the route
route_1 <- upper.tri(df)*df
...
for (i in 1:n){
  ...
  optimise <- optimise(constrained_load_per_bus,interval=c(1,3),maximum=TRUE)
